...
def pytest_addoption(parser):
    ...
    group.addoption(
        "--fail-pipelines-on-test-failure",
        action="store_true",
        dest="fail_pipelines_on_test_failure",
        default=False,
        help="Fail the Azure Pipelines build if any tests fail.",
    )
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if (
        ...
        covpath = os.path.normpath(
            os.path.abspath(
                os.path.expanduser(
                    os.path.expandvars(session.config.option.cov_report["xml"])
                )
            )
        )
        reportdir = os.path.normpath(
            os.path.abspath(session.config.getoption("report_dir"))
        )
    ...
    if (
        session.config.getoption("fail_pipelines_on_test_failure")
        and session.testsfailed > 0
    ):
        print(
            "##vso[task.complete result=Failed;]Failing Build due to {0} test(s) failed, {1} test(s) collected.".format(
                session.testsfailed, session.testscollected
            )
        )
        sys.exit(1)
...
if parse_version(pytest.__version__) >= parse_version("7.0.0"):
    ...
    def pytest_warning_recorded(warning_message, *args, **kwargs):
        print(
            "##vso[task.logissue type=warning;]{0}".format(str(warning_message.message))
        )
